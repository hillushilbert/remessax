...
class BB extends Remessa {
	...
	public function setHeader(){
		...
		$conteudo .= '0';											//01.0 001 a 001 9(001) Identifica��o do Registro Header: �0� (zero)
		$conteudo .= 1;												//02.0 002 a 002 9(001) Tipo de Opera��o: �1� (um)
		$conteudo .= 'REMESSA'; 									//03.0 003 a 009 X(007) Identifica��o por Extenso do Tipo de Opera��o #01
		$conteudo .= '01';  										//04.0 010 a 011 9(002) Identifica��o do Tipo de Servi�o: �01�
		$conteudo .= $this->formatString('COBRANCA',8); 			//05.0 012 a 019 X(008) Identifica��o por Extenso do Tipo de Servi�o: �COBRANCA�
		$conteudo .= $this->complementoRegistro(7,"brancos");		//06.0 020 a 026 X(007) Complemento do Registro: �Brancos�
		$conteudo .= $this->formatNumber($config->agencia,4);		//07.0 027 a 030 9(004) Prefixo da Ag�ncia: N�mero da Ag�ncia onde est� cadastrado o conv�nio l�der do cedente #02
		$conteudo .= $this->formatNumber($config->agencia_dv,1);	//08.0 031 a 031 X(001) D�gito Verificador - D.V. - do Prefixo da Ag�ncia. #02
		$conteudo .= $this->formatNumber($config->conta,8);			//09.0 032 a 039 9(008) N�mero da Conta Corrente: N�mero da conta onde est� cadastrado o Conv�nio L�der do Cedente #02
		$conteudo .= $this->formatNumber($config->conta_dv,1);		//10.0 040 a 040 X(001) D�gito Verificador - D.V. � do N�mero da Conta Corrente do Cedente 02
		...
		$conteudo .= $this->formatData();							//14.0 095 a 100 9(006) Data da Grava��o: Informe no formato �DDMMAA� #21
		$conteudo .= $this->formatNumber($config->codigo,7);		//15.0 101 a 107 9(007) Seq�encial da Remessa #03
		$conteudo .= $this->complementoRegistro(22,"brancos");		//16.0 108 a 129 X(22)  Complemento do Registro: �Brancos�
		$conteudo .= $this->complementoRegistro(7,"zeros");			//17.0 130 a 136 9(007) N�mero do Conv�nio L�der (numera��o acima de 1.000.000 um milh�o)" #04
		$conteudo .= $this->complementoRegistro(258,"brancos");		//18.0 137 a 394 X(258) Complemento do Registro: �Brancos�
		$conteudo .= $this->sequencial(1); 							//19.0 395 a 400 9(006) Seq�encial do Registro:�000001�
	...
	public function setMovimento(){
		...
		foreach($this->titulos as $cliente)
		{
			...
			$conteudo .= 7;												// 01.7 001 a 001 9(001) Identifica��o do Registro Detalhe: 7 (sete)
			$conteudo .= '22'; 											// 02.7 002 a 003 9(002) Tipo de Inscri��o do Cedente #22
			$conteudo .= $this->formatNumber($config->cpf_cnpj,14);		// 03.7 004 a 017 9(014) N�mero do CPF/CNPJ do Cedente
			$conteudo .= $this->formatNumber($config->agencia,4);  		// 04.7 018 a 021 9(004) Prefixo da Ag�ncia 02
			$conteudo .= $this->formatNumber($config->agencia_dv,1);	// 05.7 022 a 022 X(001) D�gito Verificador - D.V. - do Prefixo da Ag�ncia #02
			$conteudo .= $this->formatNumber($config->conta,8); 		// 06.7 023 a 030 9(008) N�mero da Conta Corrente do Cedente #02
			$conteudo .= $this->formatNumber($config->conta_dv,1);		// 07.7 031 a 031 X(001) D�gito Verificador - D.V. - do N�mero da Conta Corrente do Cedente #02
			$conteudo .= $this->formatNumber($config->cod_convenio,7);	// 08.7 032 a 038 9(007) N�mero do Conv�nio de Cobran�a do Cedente #02
			$conteudo .= $this->formatNumber($cliente->id_titulo,25);	// 09.7 039 a 063 X(025) C�digo de Controle da Empresa #23
			$conteudo .= $this->formatNumber($cliente->id_titulo,17);	// 10.7 064 a 080 9(017) Nosso-N�mero #06
			$conteudo .= $this->complementoRegistro(2,"zeros");			// 11.7 081 a 082 9(002) N�mero da Presta��o: �00� (Zeros)
			...
			$conteudo .= $this->complementoRegistro(3,"brancos"); 		// 15.7 089 a 091 X(003) Prefixo do T�tulo: �Brancos�
			$conteudo .= $this->formatNumber($config->var_carteira,3);	// 16.7 092 a 094 9(003) Varia��o da Carteira #02
			$conteudo .= $this->complementoRegistro(1,"zeros");			// 17.7 095 a 095 9(001) Conta Cau��o: �0� (Zero)
			$conteudo .= $this->complementoRegistro(6,"zeros");			// 18.7 096 a 101 9(006) N�mero do Border�: �000000� (Zeros)
			$conteudo .= $this->formatString($config->tp_cobranca,5);  // 19.7 102 a 106 X(005) Tipo de Cobran�a #24
			$conteudo .= $this->formatNumber($config->carteira,3); 		// 20.7 107 a 108 9(002) Carteira de Cobran�a #25
			$conteudo .= $this->formatNumber(1,2);						// 21.7 109 a 110 9(002) Comando #20
			22.7 111 a 120 X(010) Seu N�mero/N�mero do T�tulo Atribu�do pelo Cedente #05
			23.7 121 a 126 9(006) Data de Vencimento #08
			24.7 127 a 139 9(011)v99 Valor do T�tulo #19
			25.7 140 a 142 9(003) N�mero do Banco: �001�
			26.7 143 a 146 9(004) Prefixo da Ag�ncia Cobradora: �0000� #26
			$conteudo .= $this->complementoRegistro(1,"brancos"); 		// 27.7 147 a 147 X(001) D�gito Verificador do Prefixo da Ag�ncia Cobradora: �Brancos�
			28.7 148 a 149 9(002) Esp�cie de Titulo #07
			29.7 150 a 150 X(001) Aceite do T�tulo: #27
			30.7 151 a 156 9(006) Data de Emiss�o: Informe no formato �DDMMAA� #28
			31.7 157 a 158 9(002) Instru��o Codificada #09
			32.7 159 a 160 9(002) Instru��o Codificada #09
			33.7 161 a 173 9(011)v99 Juros de Mora por Dia de Atraso #10
			34.7 174 a 179 9(006) Data Limite para Concess�o de Desconto/Data de Opera��o do BBVendor/Juros de Mora. #11
			35.7 180 a 192 9(011)v99 Valor do Desconto #29
			36.7 193 a 205 9(011)v99 Valor do IOF/Qtde Unidade Vari�vel. #30
			37.7 206 a 218 9(011)v99 Valor do Abatimento #31
			38.7 219 a 220 9(002) Tipo de Inscri��o do Sacado #32
			39.7 221 a 234 9(014) N�mero do CNPJ ou CPF do Sacado #33
			...
			42.7 275 a 314 X(040) Endere�o do Sacado
			...
			48.7 392 a 393 X(002) N�mero de Dias Para Protesto #34
			$conteudo .= $this->complementoRegistro(1,"brancos"); 		// 49.7 394 a 394 X(001) Complemento do Registro: �Brancos�
			50.7 395 a 400 9(006) Seq�encial de Registro #35
			...
			$conteudo .= $this->complementoRegistro(4,"zeros");                     // C�D.INSTRU��O/ALEGA��O A SER CANC NOTA 27             	034 037        9(04)
			               // USO / IDENT. DO T�TULO NA EMPRESA NOTA 2            		038 062        X(25)
			...
			$conteudo .= '0000000000000';                                        	// QTDE MOEDA            NOTA 4                            	071 083        9(08)V9(5)
			                 // n� da carteira        n� carteira banco                	084 086        9(03)            
			...
			$conteudo .= $this->complementoRegistro(10,'brancos');                  // n� documento / n� documento de cobranca    NOTA 18       111 120        X(10)
			...
			$conteudo .= 341;                                                    	// codigo do banco        N� BANCO C�MARA COMP.            	140    142        9(03)         
			$conteudo .= $this->zeros(0,5);                                         //agencia cobradora / ONDE T�TULO SER� COBRADO NOTA 9    	143 147        9(05)
			...
			$conteudo .= $this->zeros(0,6);    										// desconto at�            data limite p/ descont    		174 179        9(06)
	...
	public function setTrailler(){
		/*01.5 001 a 001 9(001) Identifica��o do Registro Transa��o: �5�
02.5 002 a 003 9(002) Tipo de Servi�o: �03�
03.5 004 a 018 X(015) Identifica��o do t�tulo do cedente 36
04.5 019 a 394 X(376) Complemento do Registro: �Brancos�
05.5 395 a 400 9(006) N�mero Seq�encial do Registro no Arquivo
X
*/
		$conteudo = '';
		$conteudo .= 9;
...
/*
Nota 02
PREFIXO DA AG�NCIA, D�GITO VERIFICADOR � D.V. - DO PREFIXO DA AG�NCIA,
N�MERO DA CONTA CORRENTE, D�GITO VERIFICADOR � DV � DO N�MERO DA
CONTA CORRENTE DO CEDENTE, CARTEIRA, VARIA��O DA CARTEIRA: Os dados
necess�rios para preenchimento desses campos ser�o fornecidos pelo Banco do Brasil.

...
abstract class Remessax_Factory {
	 ...
	 * getConfig
	 *
	 * Retorna objeto stdclass com as informaчѕes de configuraчуo do banco
	...
	protected function InsertRemessa($nome_arquivo){
		...
		//throw new Exception("Remessa jс foi criada!");
	...
	public function makeRemessa($data_ini,$data_fim){
		...
		throw new Exception('Nуo foi informado a data de geraчуo do arquivo de remssa!');
		
		$this->periodo_ini = $data_ini;
		$this->periodo_fim = $data_fim;
		
		if($this->config === null)
		throw new Exception('Configuraчуo nуo informada!');
		
		if($this->aTitulos === null)
		throw new Exception('Titulos nуo informados!');
		...
		try{
			...
			// grava registros da amarraчуo dos arquivos de remessa com os titulos
